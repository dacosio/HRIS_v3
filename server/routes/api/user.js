...
router.get("/:id", authClass.authenticate(), function(req, res, next) {
  userService.get(req.params.id).then(user => res.json(user));
});
...
router.post("/", authClass.authenticate(), function(req, res, next) {
  ...
  userService.create(user).then(id => res.json(id));
  // res.render('login')
});
...
router.delete("/:id", authClass.authenticate(), function(req, res, next) {
  userService.delete(req.params.id).then(affected => res.json(affected));
});
...
router.post("/login", function(req, res) {
  ...
  if (req.body.email && req.body.password) {
    ...
    userService.login(email, password).then(users => {
